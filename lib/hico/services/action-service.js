define(['qvangular', 'qlik', 'ng!$q', 'underscore', '../prefix', '../translations/translation', '../directives/modal-dialog', './qlik-service', './config-service'], function (qvangular, qlik, $q, _, prefix, translation, Modal) {
		...
		function getActionsDefinition() {
			return {
				...
				'none': {
					...
					parameters: ['none'],
					execute: function execute() {/* No action */}
				},
				...
				'custom': {
					...
					parameters: ['custom'],
					execute: function execute($, qlik, $scope, $element, contextType, evt, params) {
						try {
				...
				'selectValues': {
					...
					execute: function execute(params) {
						var field,
						    ...
						    values = params[1].split(';').map(function (val) {
							return val.trim();
						});
						...
						if (selectedValues.length > 0) {
							if (params[4]) {
								...
								values = params[1].filter(function (val) {
						...
						return deferred.promise.then(function (field) {
					...
					execute: function execute(params) {
						try {
							...
							return qlik.currApp().doReload(parseInt(params[0]), params[1]).then(function () {
								setTimeout(function () {
									$scope.close();
								}, 1000); // Show the dialog for minimum 1s
							}).catch(function (err) {
   ...
   */
		function getTriggersDefiition() {
			return [{ type: 'click', label: 'ON_CLICK' }, { type: 'beforeUnload', label: 'BEFORE_NAVIGATION' }, { type: 'load', label: 'ON_LOAD' }, { type: 'selection', label: 'ON_SELECTION' }, {
				type: 'custom',
				label: 'CUSTOM',
   ...
   */
		function init(service) {
			$q.all([initLists().then(function () {
				updateParameters(_listLayout, _parameters);
			}), configService.getActionBlacklist().then(function (blacklist) {
				updateActions(blacklist, _actions);
			})]).then(function () {
				_ready.resolve(service);
			});
			...
			HiCo.performCustomTrigger = HiCo.performCustomTrigger || performCustomTrigger;
		}
   ...
   */
		function getActions() {
			...
			return _.isEmpty(allowedActions) ? null : allowedActions;
		}
   ...
   */
		...
		function initLists() {
			return $q.all([qlikService.listProvider.getListData('VariableList').then(function (variables) {
				_listLayout.variables = variables;
			}), qlikService.listProvider.getListData('FieldList').then(function (fields) {
				_listLayout.fields = fields;
			}), qlikService.listProvider.getListData('DimensionList').then(function (dimensions) {
				_listLayout.dimensions = dimensions;
			})]);
		}
		...
		function updateAllLists() {
			return $q.all([qlikService.listProvider.getListData('VariableList').then(function (variables) {
				...
				_listLayout.fields = fields;
			}), qlikService.listProvider.getListData('BookmarkList').then(function (bookmarks) {
				_listLayout.bookmarks = bookmarks;
			}), qlikService.listProvider.getListData('story').then(function (stories) {
				_listLayout.stories = stories;
			}), qlikService.listProvider.getListData('sheet').then(function (sheets) {
				_listLayout.sheets = sheets;
			}), qlikService.listProvider.getListData('DimensionList').then(function (dimensions) {
				_listLayout.dimensions = dimensions;
			}), qlikService.listProvider.getListData('MeasureList').then(function (measures) {
				_listLayout.measures = measures;
			}), qlikService.listProvider.getListData('MediaList').then(function (media) {
				_listLayout.media = media;
			})]);
		}
  ...
  */
	function updateParameters(listLayout, parameters) {
		...
		parameters.qMode.items = [{ value: '0', label: translation.getTranslation('DEFAULT_MODE') }, { value: '1', label: translation.getTranslation('ATTEMPT_RECOVERY_ON_ALL_ERRORS') }, { value: '2', label: translation.getTranslation('FAIL_ON_ALL_ERRORS') }];

		parameters.urlTarget.items = [{ value: 'customWindowName', label: translation.getTranslation('NAMED_WINDOW') }, { value: '_blank', label: translation.getTranslation('NEW_WINDOW') }, { value: '_self', label: translation.getTranslation('SAME_WINDOW') }];
  ...
  */
	function updateActions(blacklist, actions) {
		...
		blacklist.forEach(function (actionName) {
			// disable blacklisted actions only
