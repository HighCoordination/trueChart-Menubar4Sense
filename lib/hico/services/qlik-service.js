...
define(['qvangular', 'qlik', 'ng!$q', '../prefix'], function (qvangular, qlik, $q, prefix) {
  ...
  */
	function QlikService() {
		...
		var _service = this,
		    _app = qlik.currApp(),
		    _enigma = _app.model.enigmaModel;
		...
		function getReady() {
			return $q.all([_service.variableProvider.getReady(), _service.expressionProvider.getReady()]).then(function () {
				return _service;
			});
		}
   ...
   */
		function createGenericObjectDef(parameters) {
			var params = (typeof parameters === 'undefined' ? 'undefined' : _typeof(parameters)) === 'object' && parameters || { id: '', type: 'GenericObject' };
   ...
   */
		function getObject(qId) {
			return _enigma.getObject(qId).catch(engineErrorHandler(_enigma, 'getObject', [qId]));
		}
   ...
   */
		function getObjectLayout(qId) {
			return _app.getObject(qId).catch(engineErrorHandler(_app, 'getObject', [qId]));
		}
   ...
   */
		function getObjectProperties(data) {
			var qId = data,
			    model = (typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' ? data : null;

			if (model) {
				qId = model.id || ((model.properties || {}).qInfo || (model.layout || {}).qInfo).qId;
			}
			return _app.getObjectProperties(qId).catch(engineErrorHandler(_app, 'getObjectProperties', [qId]));
		}
   ...
   */
		function getMasterObjectProperties(data) {
			...
			return getObject.then(function (obj) {
				...
				if (qExtendsId) {
					// check if we need to get the real sourceobject
   ...
   */
		function getProperties(data, cached) {
			var model = (typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' ? data : null,
			...
			return getObjectProperties(data).then(function (obj) {
   ...
   */
		function createObject(definition) {
			return _enigma.createObject(definition).catch(engineErrorHandler(_enigma, 'createObject', [definition]));
		}
   ...
   */
		function createSessionObject(definition) {
			return _app.createGenericObject(definition).catch(engineErrorHandler(_app, 'createGenericObject', [definition]));
		}
   ...
   */
		function createSessionCube(qHyperCubeDef, hicoId) {
			console.info('createSessionCube', hicoId);
   ...
   */
		function createChildCube(model, qHyperCubeDef, hicoId) {
			console.info('createChildCube', hicoId);
			var definition = { qInfo: { qId: hicoId, qType: 'HyperCube' }, qHyperCubeDef: setMaxInitialDataFetch(qHyperCubeDef) };
   ...
   */
		function updateChildCube(qId, qHyperCubeDef) {
			...
			return getObjectProperties(qId).then(function (model) {
				...
				return setProperties(model, model.properties).then(function () {
					return model;
				});
			});
		}
   ...
   */
		function getChildItems(layout) {
			var items = layout && layout.qChildList && layout.qChildList.qItems || [];
   ...
   */
		function createChild(model, definition) {
			return model.createChild(definition).catch(engineErrorHandler(model, 'createChild', [definition]));
		}
   ...
   */
		function inPlayMode() {
			return qlik.navigation.getMode() === 'play';
		}
  ...
  */
   ...
   */
		...
		function isSupported(name) {
			...
			return qListName !== undefined ? $q.resolve(qListName) : $q.reject();
		}
   ...
   */
		...
		function getExpressionDef(expr) {
			if (typeof expr === 'string' && expr.indexOf('=') === 0) {
				return hash(expr);
			} else if ((typeof expr === 'undefined' ? 'undefined' : _typeof(expr)) === 'object' && expr.hasOwnProperty('qStringExpression')) {
   ...
   */
		function evalExpression(expression) {
			...
			return getReady().then(function () {
				...
				if (exprObj.properties.exprList && exprObj.properties.exprList.hasOwnProperty(expr.key) && exprObj.properties.exprList[expr.key].qStringExpression !== expr.def) {
   ...
   */
		function getValue(name) {
			...
			if (typeof name !== 'string') {
				return Promise.reject('Wrong parameter type: "name" must be from type "string" but is from type "' + (typeof name === 'undefined' ? 'undefined' : _typeof(name)) + '"');
			}
			if (isNew(name)) {
				addVariable(name);
   ...
   */
		function profile(evaluate) {
			getReady().then(getValueList).then(function (valueList) {
				...
				evalExp(name, key).then(function () {
					...
					console.info({
						all: sortable,
						errors: sortable.filter(function (v) {
							return v.value.indexOf('Error') > -1;
						}),
						invalid: sortable.filter(function (v) {
							return v.value === '-';
						})
					});
				});
	...
	function SelectionProvider(qlikService, app) {
		...
		function select(fieldName, values, toggle, softLock, keep) {
			...
			if (!validParams) {
				return $q.reject('No valid parameters', arguments);
			}

			if (!prevRequest || !prevRequest.pending) {
				// first request, or a finished last one
				if (keep && isSelected(fieldName)) {
					return $q.resolve(true); // Field is already selected -> keep selection -> return success = true
				}
				prevRequest = _requests[fieldName] = {
					promise: app.field(fieldName).select(values, !!toggle, !!softLock).catch(qlikService.engineErrorHandler(app.field(fieldName), 'select', [values, !!toggle, !!softLock])),
					arguments: currArgs,
					pending: true,
					counter: 0
				};
			...
			return $q.resolve(prevRequest.promise).then(function (success) {
				...
				if (!success) {
					...
					if (arraysDiffers(prevArgs[1], currArgs[1]) && !keep) {
						// Make new selection if previous select values differs
