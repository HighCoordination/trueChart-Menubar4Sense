...
define(['qlik', 'require', 'qvangular', 'ng!$compile', 'ng!$timeout', 'text!./media-library.html', './modal-dialog', '../prefix', '../translations/translation', '../services/media-service', '../../external/leonardo-ui/leonardo-ui'], function (qlik, require, qvangular, $compile, $timeout, template, Modal, prefix, translation, MediaService, leonardoui) {

	var _mediaService = MediaService.getInstance(qlik.currApp().id);
	var _mediaProvider = _mediaService.mediaProvider;
	var STATES = {
		DEFAULT: 'DEFAULT',
		HIDDEN: 'HIDDEN',
  ...
  */
	MediaLibrary.show = function (options) {
		var template = void 0,
		    element = void 0,
		    opts = (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' ? options : {},
  ...
  */
   ...
   */
		this.$onInit = function () {
			...
			updateMediaItems($scope.mediaUrl);
		};
   ...
   */
		this.previewFiles = function (fileList) {
			var $filesPreview = this.elements.$filesPreview,
			    promises = [];
   ...
   */
		...
		this.addFromUrl = function (url) {
			addUrls([url], false).then(resolveConflicts).then(function (mediaItems) {
				var url = mediaItems && mediaItems.length && mediaItems[mediaItems.length - 1].tcMedia.url;
				$timeout(function () {
				...
				buttons: [{
					...
					handler: function handler() {
						...
						for (var i = removableItems.length - 1; i >= 0; i--) {
							...
							var removeIndex = itemsToRemove.indexOf(currentItem);
							if (removeIndex !== -1) {
				...
				default:
					...
					[].slice.apply(popover.element.querySelectorAll('[data-add-media]')).forEach(function (element) {
						element.addEventListener('click', function () {
							var source = this.getAttribute('data-add-media');
							$timeout(function () {
								addMedia(source);
							});
							popover.close();
   ...
   */
		function addUrls(urls, replace) {
			...
			return Promise.all(urls.map(function (url) {
				...
				return {
					added: mediaItems.filter(function (item) {
   ...
   */
		function addRejected(mediaItems, replace) {
			return new Promise(function (resolve, reject) {
				var rejected = mediaItems.rejected;
				if (!rejected || !rejected.length) {
					resolve(mediaItems.added);
				} else {
					var add = rejected[0] instanceof File ? addFiles : addUrls;
   ...
   */
		function resolveConflicts(mediaItems) {
			var trans = $scope.trans;
			return new Promise(function (resolve, reject) {
				var rejectedItems = mediaItems.rejected;
				...
				$timeout(function () {
					var rejected = rejectedItems.map(function (item) {
