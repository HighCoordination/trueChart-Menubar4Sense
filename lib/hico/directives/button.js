define(['jquery', 'qlik', 'angular', 'qvangular', 'ng!$q', 'text!./button.html', '../prefix', '../services/media-service',
//'../../external/toastr/toastr.min',
//'css!../../external/toastr/toastr.css',
'../services/action-service', '../services/qlik-service'], function ($, qlik, angular, qvangular, $q, template, prefix, MediaService /*, toastr*/) {
  ...
  */
	function ButtonController($scope, $element, actionService, qlikService) {
		...
		this.mediaProvider = MediaService.getInstance(app.id).mediaProvider;
  ...
  */
	ButtonController.prototype = {
		...
		doAction: function doAction(evt, state, tType) {
			var $ctrl = this;
			// Don't execute actions in edit mode
			if (this.inEditMode || qlik.navigation.getMode() === 'edit' || $ctrl.actionsRunning[tType]) {
				//tType === 'click' && toastr.info('In "edit" mode the execution of button actions is disabled!');
				return;
			}

			this.actionService.getReady().then(function (actionService) {
				var contextType = { isMashUp: !qlik.navigation.inClient };
				state.triggers.forEach(function (trigger) {
					if (trigger.type === tType) {
						if (trigger.type === 'custom' && evt.name !== trigger.name) {
							return;
						}
						var actions = trigger.actions,
						    i = 0;
						// start execution
						$ctrl.actionsRunning[tType] = true;
						execute(actions, i);
					}
				});

				function execute(actions, index) {
					if (index >= 0 && index === actions.length) {
						// nothing to execute
						$ctrl.actionsRunning[tType] = false;
						return $q.resolve();
					}
					...
					try {
						if (action.name === 'custom') {
							args = [$, qlik, $ctrl.$scope, $ctrl.$element, contextType, evt, action.params];
						}
						return $q.resolve(currAction.execute.apply(currAction, args)).catch($ctrl.qlikService.engineErrorHandler(currAction, 'execute', args)).then(function () {
							return execute(actions, index);
						});
					} catch (e) {
						console.warn('Error during performing an aciton: ', [{ name: action.name, params: action.params }, e]);
						$ctrl.actionsRunning[tType] = false;
					}
					return $q.resolve();
				}
   ...
   */
		getActiveStates: function getActiveStates(states) {
			var i = 0,
			    activeStates = [];

			if (!this.evaluateStates) {
				return $q.all(this.activeStates);
			}
			...
			return $q.all(activeStates.map(evalExpressions, this));
		},

		refreshActiveStates: function refreshActiveStates() {
			this.getActiveStates(this.states).then(this.setActiveStates);
		},

		applyStyles: function applyStyles(states, hovering, defaults, scrollingInfo) {
			states.forEach(function (state) {
				...
				var width = layout.width + (!isNaN(layout.width) ? 'px' : '');
				styles['width'] = width;
				styles['min-width'] = width;

				var height = layout.height + (!isNaN(layout.height) ? 'px' : '');
				...
				if (layout.margin && layout.margin !== 'none' && (state.buttonType === 'simple' || state.buttonType === 'image' || layout.height) && !(layout.height === 'auto' || layout.height && layout.height.indexOf('calc(') === 0)) {
		...
		applyClasses: function applyClasses(states) {
			states.forEach(function (state) {
   ...
   */
		$onInit: function $onInit() {
			...
			qlik.currApp().selectionState().OnData.bind(registerOnSelectionHandler);
   ...
   */
		onDestroy: function onDestroy() {
			...
			if (app) {
				// in some cases app can be "null" (i.e. when app was closed because of conection problems)
	...
	function evalExpressions(state) {
		...
		state.text && promises.push(ctrl.qlikService.evalExpression(state.text).then(function (reply) {
			_state.text = reply;
		}));
		state.tooltip && promises.push(ctrl.qlikService.evalExpression(state.tooltip).then(function (reply) {
			_state.tooltip = reply;
		}));
		state.icon && promises.push(ctrl.qlikService.evalExpression(state.icon).then(function (reply) {
			_state.icon = reply;
		}));
		state.buttonState && promises.push(ctrl.qlikService.evalExpression(state.buttonState).then(function (reply) {
			_state.buttonState = reply !== '-' ? reply : 'normal';
		}));

		if (state.style) {
			style = state.style;
			if (style.background) {
				style.background.color && promises.push(ctrl.qlikService.evalExpression(style.background.color).then(function (reply) {
					_state.style.background.color = reply !== '-' ? reply : undefined;
				}));
				style.background.hoverColor && promises.push(ctrl.qlikService.evalExpression(style.background.hoverColor).then(function (reply) {
					_state.style.background.hoverColor = reply !== '-' ? reply : undefined;
				}));
				style.background.image && promises.push(ctrl.qlikService.evalExpression(style.background.image).then(function (reply) {
					if (reply === '-') {
						...
						return ctrl.mediaProvider.getMediaDataByUrl(reply).then(function (data) {
							_state.style.background.image = data;
						});
					}
				}));
			}
			...
			if (style.icon) {
				style.icon.color && promises.push(ctrl.qlikService.evalExpression(style.icon.color).then(function (reply) {
					_state.style.icon.color = reply !== '-' ? reply : undefined;
				}));
				style.icon.hoverColor && promises.push(ctrl.qlikService.evalExpression(style.icon.hoverColor).then(function (reply) {
					_state.style.icon.hoverColor = reply !== '-' ? reply : undefined;
				}));
			}

			if (state.style.font) {
				style.font.color && promises.push(ctrl.qlikService.evalExpression(style.font.color).then(function (reply) {
					_state.style.font.color = reply !== '-' ? reply : undefined;
				}));
				style.font.hoverColor && promises.push(ctrl.qlikService.evalExpression(style.font.hoverColor).then(function (reply) {
					_state.style.font.hoverColor = reply !== '-' ? reply : undefined;
				}));
				style.font.family && promises.push(ctrl.qlikService.evalExpression(style.font.family).then(function (reply) {
					_state.style.font.family = reply !== '-' ? reply : undefined;
				}));
				style.font.size && promises.push(ctrl.qlikService.evalExpression(style.font.size).then(function (reply) {
					_state.style.font.size = reply !== '-' ? reply : undefined;
				}));
				style.font.weight && promises.push(ctrl.qlikService.evalExpression(style.font.weight).then(function (reply) {
					_state.style.font.weight = reply !== '-' ? reply : undefined;
				}));
				style.font.style && promises.push(ctrl.qlikService.evalExpression(style.font.style).then(function (reply) {
					_state.style.font.style = reply !== '-' ? reply : undefined;
				}));
			}

			if (style.border) {
				style.border.hoverColor && promises.push(ctrl.qlikService.evalExpression(style.border.hoverColor).then(function (reply) {
					_state.style.border.hoverColor = reply !== '-' ? reply : undefined;
				}));
				style.border.color && promises.push(ctrl.qlikService.evalExpression(style.border.color).then(function (reply) {
					_state.style.border.color = reply !== '-' ? reply : undefined;
				}));
			}

			if (style.custom) {
				promises.push(evalExpr(style.custom).then(function (reply) {
					_state.style.custom = reply !== '-' ? reply : undefined;
				}));
			}
		}
  ...
  */
	function isTrue(condition) {
		...
		return condition === '' || condition === 'true' || condition === '1' || condition === '-1';
	}

	Button.Trigger = function () {
		this.type = 'click';
		this.actions = [new Button.Action()];
	};
