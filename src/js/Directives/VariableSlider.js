...
define(['qvangular', '../../templates/variableSlider.html'], function(qvangular, template){
					...
					'$scope', '$element', function($scope, $element){
							...
							if(UtilService.isValidVariable(val)){
								$timeout(() =>{
									const appearance = $scope.layout.appearance,
										...
										variableSliderBackground = appearance.variableSliderBackground,
										variableSliderTrack = appearance.variableSliderTrack,
										variableSliderHandle = appearance.variableSliderHandle,
						...
						$scope.$watchGroup(['item.variableValueStart', 'item.variableValueEnd'], (val) =>{
							if(UtilService.isValidVariable(val[0]) && UtilService.isValidVariable(val[1])){
								$timeout(() =>{
									...
									let trackStyle = [
											{background: appearance.variableSliderTrack},
											{background: appearance.variableSliderTrack}
										],
										handleStyle = [
											{background: appearance.variableSliderHandle, border: 0},
											{background: appearance.variableSliderHandle, border: 0}
										];
						...
						$scope.renderSlider = function(defaultValue, trackStyle, handleStyle){
							...
							RangeSlider.show({
								...
								railStyle: {background: appearance.variableSliderBackground},
								trackStyle: trackStyle,
								handleStyle: handleStyle,
								dotStyle: {background: appearance.variableSliderSteps, border: 0},
								activeDotStyle: {
									background: type === 'single' ? appearance.variableSliderSteps : appearance.variableSliderActiveSteps,
									border: 0,
								},
							})
						};
						...
						function onChangeMultiWatcher(){
							...
							$timeout(() =>{
								...
								$scope.item.multiVariables.forEach((variable) =>{
									...
									trackStyle.push({background: appearance.variableSliderTrack});
									handleStyle.push({background: appearance.variableSliderHandle, border: 0});
								});
