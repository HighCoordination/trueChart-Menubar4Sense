define([
	...
	function MediaService(appId){
		...
		function getReady(){
			return Promise.all(
				[
					_instance.mediaProvider.getReady()
				]
			).then(function(){
				return _instance;
			});
	...
	function MediaProvider(qlikService){
		...
		function init(){
			...
			qlikService.getObject(storeId).then(function(tcMediaStore){
				if(!tcMediaStore){ // is null, when object not found
		...
		function getMedia(name){
			return getReady().then(function(){
				if(!_cache[name] && _mediaItems.length > 0){
					let mediaItem = getMediaItemByName(name);
					if(mediaItem){
		...
		function createMedia(name, props){
			return new Promise(function(resolve, reject){
				let mediaDef = createTCMediaDef(name, props);
		...
		function createTCMediaDef(name, props){
			let def = qlikService.createGenericObjectDef({type: 'tcMedia'});
